...
@section('content')
	...
	<section class="content-header">					
        <div class="container-fluid my-2">
        	<div class="row mb-2">
        		<div class="col-sm-6">
        			<h1>Sản phẩm bán trong quý 3 năm <?= date('Y'); ?></h1>
        		</div>
						...
						<div class="card">
							...
							<div class="card-body p-0">								
								<table id="myTable" class="table table-bordered table-hover">
									...
									<tbody>
                                        @if (!empty($SellingProductsQuarter3))
                                            @foreach ($SellingProductsQuarter3 as $item)
												<tr>
                                            ...
                                            @endforeach
                                        @else
                                            <td colspan="5" class="text-center">Chưa đến thời gian bắt đầu quý 4</td>
                                        @endif
									</tbody>
								</table>										
							</div>
							<div class="card-footer clearfix">
								{{$SellingProductsQuarter3->links()}}
							</div>
						</div>
					</div>
					<!-- /.card -->
